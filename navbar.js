...
function updateNavbarLogin(user) {
  ...
  if (user) {
    ...
    var nama   = profil.namaDepan || (user.user_metadata && user.user_metadata.full_name) || user.email.split('@')[0];

    // Buat dropdown akun (hanya sekali)
    if (akunLink && !document.getElementById('akunDropdown')) {
      akunLink.innerHTML =
      ...
      var html =
        '<div style="padding:14px 16px;background:linear-gradient(135deg,#1b5e20,#2e7d32);border-bottom:1px solid #e8f5e9">' +
          ...
          '<div style="font-size:.72rem;color:rgba(255,255,255,.7)">' + (user.email || '') + '</div>' +
        '</div>';
...
// Tunggu Supabase siap lalu pasang onAuthStateChange
function initNavbarAuth() {
  if (!window.sb) { setTimeout(initNavbarAuth, 200); return; }

  // Cek session saat ini (termasuk OAuth callback)
  window.sb.auth.getSession().then(function(res) {
    var user = res.data && res.data.session && res.data.session.user;
    updateNavbarLogin(user || null);

    // Jika ini halaman login dan sudah ada session (dari OAuth callback), redirect ke home
  ...
  // Realtime auth state changes (login/logout/OAuth callback)
  window.sb.auth.onAuthStateChange(function(event, session) {
    var user = session && session.user;
    updateNavbarLogin(user || null);

    // Setelah OAuth callback berhasil, redirect ke home
